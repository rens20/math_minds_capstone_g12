...
$sql = "SELECT id, question, answer_a, answer_b, answer_c, answer_d, correct_answer, image_path FROM medium_questions LIMIT 10";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['undo'])) {
        ...
        header('Location: medium.php?id=' . urlencode($_SESSION['user_id']));
        exit;
        ...
        if ($_SESSION['current_question'] >= count($questions)) {
            header('Location: quiz_result2.php?id=' . urlencode($_SESSION['user_id']));
            exit;
        }
    } else {
