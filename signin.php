...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
 ...
 if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $user = ValidateLogin($email, $password); 
     ...
     if($user) {
        $_SESSION['login_success'] = true;  // Set session variable upon successful login
        if ($user['type'] == 'admin') {
            header("Location: ./admin_easy.php?id=" . urlencode($user['id']));
            exit();
        } else {
            $_SESSION['username'] = $user['name'];
            echo '<script>alert("Login successful!");</script>'; // Display success alert using JavaScript
            echo '<script>window.location.href = "./gamepage.php?id=' . urlencode($user['id']) . '&name=' . urlencode($user['name']) . '";</script>'; // Redirect using JavaScript
            exit();
        } 
    } else {
